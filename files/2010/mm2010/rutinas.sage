...
def NewtonRaphson(e,seed,f,f_): #seed es x_0, f_ es derivada
    ...
    print "x_i=",next.n(digits=9), "f(x_i)=" , f(seed).n(digits=9), " f'(x)=", f_(seed), " error relativo: ", abs((next-seed)/next)
    if abs(next-seed)<e:
        return next
    else:
...
def secanteLibroOptimizada(f,x0,x1,e,numeroIteraciones):
    ...
    while (contador <= numeroIteraciones):
        ...
        print "next x: ", nextX.n(digits=juanferGay) , " f(x)", f(nextX).n(digits=juanferGay), " error relativo: ",  abs((nextX-x1)/nextX).n(digits=6)
        if (abs(nextX-x1) < e):
