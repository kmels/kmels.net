...
The CYK algorithm builds a matrix from a context free grammar `G` in chomsky nomral form and a word `w` to determine if that word is in the language generated by G, L(G). 
> import Data.List
> import Data.Maybe
...
>   setOfSymbols = trace ("Matrix: " ++ show matrix) $ matrix !! i !! j
